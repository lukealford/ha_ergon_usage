...
async def test_index_html_renders_status(aiohttp_client_factory, coordinator):
    ...
    # Run now is AJAX: no form POST redirect; phase polling updates the UI.
...
@pytest.mark.asyncio
async def test_index_adopts_ha_theme_variables(aiohttp_client_factory, coordinator):
    # The page includes a runtime theme bridge that reads the parent HA
    # frame's CSS custom properties so any HA theme applies.
    client = await aiohttp_client_factory(create_app(coordinator))
    response = await client.get("/")
    text = await response.text()
    assert "--primary-color" in text
    assert "--card-background-color" in text
