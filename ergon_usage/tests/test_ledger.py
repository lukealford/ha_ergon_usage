...
def test_rate_observations_are_persisted_without_creating_effective_periods(ledger):
    observed = START + timedelta(minutes=15)
    rate = TariffRate(ACCOUNT, TARIFF, observed, Decimal("0.28895"), Decimal("1.80508"))

    changed = ledger.record_rates([rate])
    repeated = ledger.record_rates([rate])

    assert (changed.changed, changed.unchanged, changed.earliest_changed) == (1, 0, observed)
